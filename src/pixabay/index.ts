...
export async function downloadVideo(folder: string, id: string) {
  ...
  const result = await axios.get("https://pixabay.com/api/videos/", { params: { key: process.env.PIXABAY, id: id || randomVideos[Math.floor(Math.random() * randomVideos.length)] } })
